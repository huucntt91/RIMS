        ...
        <c:if test="${nodeList != null}">
            ...
            var nodesData = [
                <c:forEach var="item" items="${nodeList}">
                    ...
                    {id: 8888888, ip: '',tinhTpId: '${tnode1.tinhTpId}',maTinh: '${tnode1.maTinh}', shape: 'image', label: '${node1.code}', image : DIR + 'bts_topo.png' , topo: ${tnode1.typeId} },
                    {id: 9999999, ip: '',tinhTpId: '${tnode2.tinhTpId}',maTinh: '${tnode2.maTinh}', shape: 'image', label: '${node2.code}', image : DIR + 'bts_topo.png' , topo: ${tnode2.typeId} }
            ];
        
            var edgesData = [
                <c:forEach var="item" items="${tlinkList}">
                     {id:${item.rownum}, tid: ${item.id}, loai: ${item.loai}, from: ${item.nodeId1}, to: ${item.nodeId2}, label: '${item.distanceM} ; ${item.bandWidth} ${item.bandWidthDv}' , smooth: { roundness: 0 , type: 'curvedCW'} , hidden: false, type: ${item.type} },
                </c:forEach>
            ...
            nodesData = getPotionNodeData(${tnode1.id}, ${tnode2.id});
