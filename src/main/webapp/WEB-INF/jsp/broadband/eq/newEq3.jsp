        ...
        <div class="col-xs-12">
            <div class="box">
                ...
                <div class="box-header">
                    <h3 class="box-title">Cập nhật thông tin Eq3</h3>
                </div>
                <div class="box-body" id="mydiv" >                                    
                    <form:form method="post" action="${pageContext.request.contextPath}/eq/updateEq3" commandName="tEq3Form">
                        <form:hidden path="id" value="${tEq3Form.id}"/>
                        <div class="form-group">
                            <div class="input-group">
                                <label class=" input-group-addon" for="exampleInputEmail1">TEq2</label>
                                <input type="hidden" name="id" value="${tEq3Form.id}" id="id" required />
                                <input type="hidden" name="tnodeId" value="${tnodeId}" id="tnodeId" required />
                                <input type="hidden" name="eq2Id" value="${tEq3Form.eq2Id}" id="eq2Id" required />
                                <form:input readonly="readonly" disabled="disabled" required="required"  path="eq2Name"
                                            class="form-control" id="eq2Name"
                                       value="${tEq3Form.eq2Name}" />   
    ...
    $(document).ready(function() {        
        ...
        $('#myModalTnode1 iframe').on('load', function(e) {
            ...
            iframe1.find('#example1 tbody').on('click', 'tr', function() {
                $('#eq2Id').val($(this).find('input.txtid').val());
            });
            iframe1.find('#example1 tbody').on('dblclick', 'tr', function() {
                //alert($(this).text());
                $('#eq2Id').val($(this).find('input.txtid').val());
                $('#eq2Name').val($(this).find('input.txtname').val());
