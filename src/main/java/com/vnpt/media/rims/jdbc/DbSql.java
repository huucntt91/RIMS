...
import static com.vnpt.media.rims.jdbc.SQLTemplate.NULL;
...
//            logger.error(e.getMessage(), e);
    ...
    public String runProc( String sqlText,List<Object> hostVariableList) throws Exception {
...
//        return pStmtReturn;
    ...
    public String runProcOutString( String sql,List<Object> hostVariableList) throws Exception {
        CallableStatement cstmt_ = null;

        try {
            cstmt_ = this.getPrepareCall(sql,hostVariableList);         
            cstmt_.registerOutParameter(1, OracleTypes.VARCHAR);

            cstmt_.execute();

            String stringResult_ = cstmt_.getString(1);


            return stringResult_;
        } catch (SQLException e) {
//            logger.error(e.getMessage(), e);
            throw new JdbcException(e);
        } finally {
            DatabaseUtils.close(null, cstmt_);
        }
    }
}
