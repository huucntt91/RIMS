package com.vnpt.media.rims.common.utils.numbers;

import com.vnpt.media.rims.common.Constants;
import com.vnpt.media.rims.jdbc.support.SerializableResultSetMetaData;
...
public class DataArrayList implements Serializable {
	...
	public short getShort(int columnIndex, int rowNbr) throws SQLException {
		int pos = this.getArrayOccurance(columnIndex);
		if (_javaType[pos] != Constants.JAVA_SHORT) {
			throw new SQLException("Invalid for columns of type: "
					+ _metaData.getColumnTypeName(columnIndex));
		}
	...
	public int getInt(int columnIndex, int rowNbr) throws SQLException {
		int pos = this.getArrayOccurance(columnIndex);
		if (_javaType[pos] != Constants.JAVA_INT) {
			throw new SQLException("Invalid for columns of type: "
					+ _metaData.getColumnTypeName(columnIndex));
		}
	...
	public float getFloat(int columnIndex, int rowNbr) throws SQLException {
		int pos = this.getArrayOccurance(columnIndex);
		if (_javaType[pos] != Constants.JAVA_FLOAT) {
			throw new SQLException("Invalid for columns of type: "
					+ _metaData.getColumnTypeName(columnIndex));
		}
	...
	public double getDouble(int columnIndex, int rowNbr) throws SQLException {
		int pos = this.getArrayOccurance(columnIndex);
		if (_javaType[pos] != Constants.JAVA_DOUBLE) {
			throw new SQLException("Invalid for columns of type: "
					+ _metaData.getColumnTypeName(columnIndex));
		}
	...
	public Date getDate(int columnIndex, int rowNbr) throws SQLException {
		int pos = this.getArrayOccurance(columnIndex);
		if (_javaType[pos] != Constants.JAVA_DATE) {
			throw new SQLException("Invalid for columns of type: "
					+ _metaData.getColumnTypeName(columnIndex));
		}
			...
			for (int i = 0; i < _metaData.getColumnCount(); i++) {
				...
				case Constants.JAVA_STRING: {
					...
					case -100: // Work-around for defect in Oracle 9iR1 JDBC
						// driver.
						// This can be removed when Oracle 9iR2 driver
						// installed.
					case Types.DATE: {
	...
	private void buildArrayMetaData() throws SQLException { // assumes _metaData
		...
		for (int i = 0; i < _metaData.getColumnCount(); i++) {
			...
			case Types.NUMERIC:
			case Types.REAL: {
				_javaType[i] = Constants.JAVA_DOUBLE;
			...
			case Types.FLOAT: {
				_javaType[i] = Constants.JAVA_FLOAT;
			...
			case Types.INTEGER: {
				_javaType[i] = Constants.JAVA_INT;
			...
			case Types.SMALLINT:
			case Types.TINYINT: {
				_javaType[i] = Constants.JAVA_SHORT;
			...
			case Types.VARCHAR: {
				_javaType[i] = Constants.JAVA_STRING;
			...
			{
				_javaType[i] = Constants.JAVA_DATE;
