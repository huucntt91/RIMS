 ...
 */
@Controller
@RequestMapping(value = "/msc")
public class MscController {

    private static Logger logger = LogManager.getLogger(MscController.class);

    private static final String LIST = "core/msc/list";
    private static final String VIEW = "core/msc/edit";
    private static final String POPUP = "core/msc/popup";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init msc");
        name = name == null ? "" : name;
        page = page == null ? "1" : page;
        Integer pageInt = Integer.parseInt(page);

        CoreFacade facade = new CoreFacade();
        int totalRows = facade.getTotalMscInfo(name, "");
        ...
        if (pageInt < 1) {
            pageInt = 1;
            return ("redirect:/msc/init?page=" + pageInt);
        ...
        List<MscInfoBO> lst = facade.findAllMscInfo(startRow, endRow, name, "");
...
@RequestMapping(value = "/popup/{nodeId}", method = RequestMethod.GET)
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CoreFacade facade = new CoreFacade();
                List<MscInfoBO> cpBO = facade.findAllMscInfo(1, 10, null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") MscInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/msc/preAdd";
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addMscInfo(item, user.getId());
        ...
        return "redirect:/msc/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") MscInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/msc/view/" + item.getNodeId();
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateMscInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CoreFacade coreFacade = new CoreFacade();
            List<MscInfoBO> mscInfo = coreFacade.findAllMscInfo(1, 10, "", nodeId);
            if (mscInfo.size() > 0) {
                coreFacade.deleteMscInfo(Long.valueOf(nodeId));
