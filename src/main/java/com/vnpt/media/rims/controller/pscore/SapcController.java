 ...
 */
@Controller
@RequestMapping(value = "/sapc")
public class SapcController {

    private static Logger logger = LogManager.getLogger(SapcController.class);

    private static final String LIST = "core_ps/sapc/list";
    private static final String VIEW = "core_ps/sapc/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init sapc");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<SapcInfoBO> lst = facade.findAllSapcInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        SapcInfoBO item = new SapcInfoBO();
        item = (SapcInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new SapcInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<SapcInfoBO> cpBO = facade.findAllSapcInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") SapcInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addSapcInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/sapc/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") SapcInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/sapc/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateSapcInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<SapcInfoBO> ggsnInfo = coreFacade.findAllSapcInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteSapcInfo(Long.valueOf(nodeId));
