 ...
 */
@Controller
@RequestMapping(value = "/sdp")
public class SdpController {

    private static Logger logger = LogManager.getLogger(SdpController.class);

    private static final String LIST = "core_ps/sdp/list";
    private static final String VIEW = "core_ps/sdp/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init sdp");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<SdpInfoBO> lst = facade.findAllSdpInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        SdpInfoBO item = new SdpInfoBO();
        item = (SdpInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new SdpInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<SdpInfoBO> cpBO = facade.findAllSdpInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") SdpInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addSdpInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/sdp/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") SdpInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/sdp/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateSdpInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<SdpInfoBO> ggsnInfo = coreFacade.findAllSdpInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteSdpInfo(Long.valueOf(nodeId));
