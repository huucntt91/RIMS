...
@Controller
@RequestMapping(value = "/trangthaicsht")
public class DMTrangThaiCshtController {

    private static final Logger logger = LogManager.getLogger(DMTrangThaiCshtController.class);
    private static final String LIST = "categories/trangthaicsht/list";
    private static final String FORM = "categories/trangthaicsht/form";
    ...
    public String init(ModelMap mm, HttpServletRequest request) {
        ArrayList<DMTrangThaiCshtBO> list = TrangThaiCshtFacade.fc_find_all("");
        mm.put("list", list);
        return LIST;
    }
    ...
    public String preAdd(ModelMap mm) throws Exception {
        DMTrangThaiCshtBO model = new DMTrangThaiCshtBO();
    ...
    public String view(@PathVariable(value = "id") String id,
        ...
        try {
            if (StringUtils.hasText(id)) {
                ArrayList<DMTrangThaiCshtBO> list = TrangThaiCshtFacade.fc_find_all(id);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "model") DMTrangThaiCshtBO model,
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("model", model);
                return "redirect:/trangthaicsht/preAdd";
            }
            if (TrangThaiCshtFacade.fn_modify("add", model.trang_thai_csht_id, model.ten_trang_thai_csht) > 0) {
                attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
                return "redirect:/trangthaicsht/init";
            } else {
                String msg = messageSource.getMessage("admin.common.error", null, locale);
                attr.addFlashAttribute("info", new Message(Message.TYPE_DANGER, Message.HEAD_DANGER, msg));
                return "redirect:/trangthaicsht/preAdd";
    ...
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "model") DMTrangThaiCshtBO model,
            BindingResult bindingResult, RedirectAttributes attr) throws Exception {
        Locale locale = LocaleContextHolder.getLocale();
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("model", model);
                return "redirect:/trangthaicsht/view/" + model.trang_thai_csht_id;
            }
            if (TrangThaiCshtFacade.fn_modify("edit", model.trang_thai_csht_id, model.ten_trang_thai_csht) > 0) {
                attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
                return "redirect:/trangthaicsht/init";
            } else {
                String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "Id") String Id,
        ...
        try {
            if (TrangThaiCshtFacade.fn_modify("del", Id, "") > -1) {
