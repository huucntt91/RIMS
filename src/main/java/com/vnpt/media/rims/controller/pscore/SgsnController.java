 ...
 */
@Controller
@RequestMapping(value = "/sgsn")
public class SgsnController {

    private static Logger logger = LogManager.getLogger(SgsnController.class);

    private static final String LIST = "core_ps/sgsn/list";
    private static final String VIEW = "core_ps/sgsn/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init sgsn");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<SgsnInfoBO> lst = facade.findAllSgsnInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        ...
        item = (SgsnInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new SgsnInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<SgsnInfoBO> cpBO = facade.findAllSgsnInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") SgsnInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addSgsnInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/sgsn/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") SgsnInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/sgsn/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateSgsnInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<SgsnInfoBO> sgsnInfo = coreFacade.findAllSgsnInfo("", nodeId);
            if (sgsnInfo.size() > 0) {
                coreFacade.deleteSgsnInfo(Long.valueOf(nodeId));
