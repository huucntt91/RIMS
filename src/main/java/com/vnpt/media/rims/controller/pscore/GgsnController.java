 ...
 */
@Controller
@RequestMapping(value = "/ggsn")
public class GgsnController {

    private static Logger logger = LogManager.getLogger(GgsnController.class);

    private static final String LIST = "core_ps/ggsn/list";
    private static final String VIEW = "core_ps/ggsn/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init ggsn");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<GgsnInfoBO> lst = facade.findAllGgsnInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        ...
        item = (GgsnInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new GgsnInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<GgsnInfoBO> cpBO = facade.findAllGgsnInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") GgsnInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addGgsnInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/ggsn/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") GgsnInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/ggsn/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateGgsnInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<GgsnInfoBO> ggsnInfo = coreFacade.findAllGgsnInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteGgsnInfo(Long.valueOf(nodeId));
