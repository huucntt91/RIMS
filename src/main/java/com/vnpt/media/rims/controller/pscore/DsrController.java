...
public class DsrController {
    ...
    private static Logger logger = LogManager.getLogger(DsrController.class);

    private static final String LIST = "core_ps/dsr/list";
    private static final String VIEW = "core_ps/dsr/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init dsr");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<DsrInfoBO> lst = facade.findAllDsrInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        DsrInfoBO item = new DsrInfoBO();
        item = (DsrInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new DsrInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<DsrInfoBO> cpBO = facade.findAllDsrInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") DsrInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addDsrInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/dsr/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") DsrInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/dsr/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateDsrInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<DsrInfoBO> ggsnInfo = coreFacade.findAllDsrInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteDsrInfo(Long.valueOf(nodeId));
