...
public class MssController {
    ...
    private static Logger logger = LogManager.getLogger(MssController.class);

    private static final String LIST = "core/mss/list";
    private static final String VIEW = "core/mss/edit";
    private static final String POPUP = "core/mss/popup";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        int totalRows = facade.getTotalMssInfo(name, "");
        ...
        List<MssInfoBO> lst = facade.findAllMssInfo(startRow,endRow, name, "");
...
@RequestMapping(value = "/popup/{nodeId}", method = RequestMethod.GET)
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CoreFacade facade = new CoreFacade();
                List<MssInfoBO> cpBO = facade.findAllMssInfo(1, 10, null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") MssInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/mss/preAdd";
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addMssInfo(item, user.getId());
        ...
        return "redirect:/mss/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") MssInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/mss/view/" + item.getNodeId();
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateMssInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CoreFacade coreFacade = new CoreFacade();
            List<MssInfoBO> mssInfo = coreFacade.findAllMssInfo(1, 10, "", nodeId);
            if (mssInfo.size() > 0) {
                coreFacade.deleteMssInfo(Long.valueOf(nodeId));
