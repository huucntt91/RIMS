 ...
 */
@Controller
@RequestMapping(value = "/mca")
public class McaController {

    private static Logger logger = LogManager.getLogger(McaController.class);

    private static final String LIST = "core_ps/mca/list";
    private static final String VIEW = "core_ps/mca/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init mca");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<McaInfoBO> lst = facade.findAllMcaInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        McaInfoBO item = new McaInfoBO();
        item = (McaInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new McaInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<McaInfoBO> cpBO = facade.findAllMcaInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") McaInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addMcaInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/mca/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") McaInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/mca/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateMcaInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<McaInfoBO> ggsnInfo = coreFacade.findAllMcaInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteMcaInfo(Long.valueOf(nodeId));
