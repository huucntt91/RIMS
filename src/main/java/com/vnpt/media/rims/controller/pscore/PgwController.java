 ...
 */
@Controller
@RequestMapping(value = "/pgw")
public class PgwController {

    private static Logger logger = LogManager.getLogger(PgwController.class);

    private static final String LIST = "core_ps/pgw/list";
    private static final String VIEW = "core_ps/pgw/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init pgw");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<PgwInfoBO> lst = facade.findAllPgwInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        ...
        item = (PgwInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new PgwInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<PgwInfoBO> cpBO = facade.findAllPgwInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") PgwInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addPgwInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/pgw/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") PgwInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/pgw/view/" + item.getNodeId();
            }
            ...
            coreFacade.updatePgwInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<PgwInfoBO> pgwInfo = coreFacade.findAllPgwInfo("", nodeId);
            if (pgwInfo.size() > 0) {
                coreFacade.deletePgwInfo(Long.valueOf(nodeId));
