 ...
 */
@Controller
@RequestMapping(value = "/stp")
public class StpController {

    private static Logger logger = LogManager.getLogger(StpController.class);

    private static final String LIST = "core/stp/list";
    private static final String VIEW = "core/stp/edit";
    private static final String POPUP = "core/stp/popup";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init stp");
        name = name == null ? "" : name;
        page = page == null ? "1" : page;
        Integer pageInt = Integer.parseInt(page);

        CoreFacade facade = new CoreFacade();
        int totalRows = facade.getTotalStpInfo(name, "");
        ...
        if (pageInt < 1) {
            pageInt = 1;
            return ("redirect:/stp/init?page=" + pageInt);
        ...
        List<StpInfoBO> lst = facade.findAllStpInfo(startRow,endRow, name, "");
...
@RequestMapping(value = "/popup/{nodeId}", method = RequestMethod.GET)
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CoreFacade facade = new CoreFacade();
                List<StpInfoBO> cpBO = facade.findAllStpInfo(1, 10, null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") StpInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/stp/preAdd";
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addStpInfo(item, user.getId());
        ...
        return "redirect:/stp/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") StpInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/stp/view/" + item.getNodeId();
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateStpInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CoreFacade coreFacade = new CoreFacade();
            List<StpInfoBO> stpInfo = coreFacade.findAllStpInfo(1, 10, "", nodeId);
            if (stpInfo.size() > 0) {
                coreFacade.deleteStpInfo(Long.valueOf(nodeId));
