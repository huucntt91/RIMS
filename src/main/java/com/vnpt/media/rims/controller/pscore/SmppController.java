 ...
 */
@Controller
@RequestMapping(value = "/smpp")
public class SmppController {

    private static Logger logger = LogManager.getLogger(SmppController.class);

    private static final String LIST = "core_ps/smpp/list";
    private static final String VIEW = "core_ps/smpp/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init smpp");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<SmppInfoBO> lst = facade.findAllSmppInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        SmppInfoBO item = new SmppInfoBO();
        item = (SmppInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new SmppInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<SmppInfoBO> cpBO = facade.findAllSmppInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") SmppInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addSmppInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/smpp/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") SmppInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/smpp/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateSmppInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<SmppInfoBO> ggsnInfo = coreFacade.findAllSmppInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteSmppInfo(Long.valueOf(nodeId));
