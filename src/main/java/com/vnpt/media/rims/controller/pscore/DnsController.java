...
public class DnsController {
    ...
    private static Logger logger = LogManager.getLogger(DnsController.class);

    private static final String LIST = "core_ps/dns/list";
    private static final String VIEW = "core_ps/dns/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init dns");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<DnsInfoBO> lst = facade.findAllDnsInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        DnsInfoBO item = new DnsInfoBO();
        item = (DnsInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new DnsInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<DnsInfoBO> cpBO = facade.findAllDnsInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") DnsInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addDnsInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/dns/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") DnsInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/dns/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateDnsInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<DnsInfoBO> ggsnInfo = coreFacade.findAllDnsInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteDnsInfo(Long.valueOf(nodeId));
