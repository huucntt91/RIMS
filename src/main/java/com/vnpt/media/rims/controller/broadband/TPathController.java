...
import com.vnpt.media.rims.facade.TPathFacade;
 ...
 */
@Controller
@RequestMapping(value = "/tPath")
public class TPathController {

    private static Logger logger = LogManager.getLogger(TPathController.class);

    private static final String LIST = "broadband/tPath/list";
    private static final String VIEW = "broadband/tPath/edit";
    private static final String POPUP = "broadband/tPath/popup";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init tPath");
        name = name == null ? "" : name;
        page = page == null ? "1" : page;
        Integer pageInt = Integer.parseInt(page);

        TPathFacade facade = new TPathFacade();
        int totalRows = facade.getTotalTPathInfo(name, "");
        ...
        if (pageInt < 1) {
            pageInt = 1;
            return ("redirect:/tPath/init?page=" + pageInt);
        ...
        List<TPathInfoBO> lst = facade.findAllTPathInfo(startRow,endRow, name, "");
    ...
    public String preAdd(ModelMap mm)  {
        TPathInfoBO item = new TPathInfoBO();
        mm.addAttribute("item", item);
        return VIEW;
    }

    @RequestMapping(value = "/view/{tPathId}", method = RequestMethod.GET)
    public String view(@PathVariable(value = "tPathId") String tPathId,
            Locale locale, RedirectAttributes attr, ModelMap mm)  {
        try {
            if (StringUtils.hasText(tPathId)) {
                TPathFacade facade = new TPathFacade();
                List<TPathInfoBO> cpBO = facade.findAllTPathInfo(1, 10, null, tPathId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") TPathInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request)  {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/tPath/preAdd";
            }
            TPathFacade coreFacade = new TPathFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addTPathInfo(item, user.getId());
        ...
        return "redirect:/tPath/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") TPathInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request)  {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/tPath/view/" + item.gettPathId();
            }
            TPathFacade coreFacade = new TPathFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateTPathInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            TPathFacade coreFacade = new TPathFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            List<TPathInfoBO> tPathInfo = coreFacade.findAllTPathInfo(1, 10, "", nodeId);
            if (tPathInfo.size() > 0) {
                coreFacade.deleteTPathInfo(Long.valueOf(nodeId), user.getId());
