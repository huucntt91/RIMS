...
@Controller
@RequestMapping(value = "/loaicongnghe")
public class DMLoaiCongNgheController {

    private static final Logger logger = LogManager.getLogger(DMLoaiCongNgheController.class);
    private static final String LIST = "categories/loaicongnghe/list";
    private static final String FORM = "categories/loaicongnghe/form";
    ...
    public String init(ModelMap mm, HttpServletRequest request) {
        ArrayList<DMLoaiCongNgheBO> list = LoaiCongNgheFacade.fc_find_all("");
        mm.put("list", list);
        return LIST;
    }
    ...
    public String preAdd(ModelMap mm) throws Exception {
        DMLoaiCongNgheBO model = new DMLoaiCongNgheBO();
    ...
    public String view(@PathVariable(value = "id") String id,
        ...
        try {
            if (StringUtils.hasText(id)) {
                ArrayList<DMLoaiCongNgheBO> list = LoaiCongNgheFacade.fc_find_all(id);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "model") DMLoaiCongNgheBO model,
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("model", model);
                return "redirect:/loaicongnghe/preAdd";
            }
            if (LoaiCongNgheFacade.fn_modify("add", model.id_loai_cong_nghe, model.ten_loai_cong_nghe) > 0) {
                attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
                return "redirect:/loaicongnghe/init";
            } else {
                String msg = messageSource.getMessage("admin.common.error", null, locale);
                attr.addFlashAttribute("info", new Message(Message.TYPE_DANGER, Message.HEAD_DANGER, msg));
                return "redirect:/loaicongnghe/preAdd";
    ...
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "model") DMLoaiCongNgheBO model,
            BindingResult bindingResult, RedirectAttributes attr) throws Exception {
        Locale locale = LocaleContextHolder.getLocale();
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("model", model);
                return "redirect:/loaicongnghe/view/" + model.id_loai_cong_nghe;
            }
            if (LoaiCongNgheFacade.fn_modify("edit", model.id_loai_cong_nghe, model.ten_loai_cong_nghe) > 0) {
                attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
                return "redirect:/loaicongnghe/init";
            } else {
                String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "Id") String Id,
        ...
        try {
            if (LoaiCongNgheFacade.fn_modify("del", Id, "") > -1) {
