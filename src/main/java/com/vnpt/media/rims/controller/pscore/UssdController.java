...
public class UssdController {
    ...
    private static Logger logger = LogManager.getLogger(UssdController.class);

    private static final String LIST = "core_ps/ussd/list";
    private static final String VIEW = "core_ps/ussd/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init ussd");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<UssdInfoBO> lst = facade.findAllUssdInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        UssdInfoBO item = new UssdInfoBO();
        item = (UssdInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new UssdInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<UssdInfoBO> cpBO = facade.findAllUssdInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") UssdInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addUssdInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/ussd/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") UssdInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/ussd/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateUssdInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<UssdInfoBO> ggsnInfo = coreFacade.findAllUssdInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteUssdInfo(Long.valueOf(nodeId));
