...
import com.vnpt.media.rims.facade.FirewallDirFacade;
 ...
 */
@Controller
@RequestMapping(value = "/firewallDir")
public class FirewallDirController {

    private static Logger logger = LogManager.getLogger(FirewallDirController.class);

    private static final String LIST = "broadband/dmFirewall/list";
    private static final String VIEW = "broadband/dmFirewall/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init firewallDir");
        name = name == null ? "" : name;
        page = page == null ? "1" : page;
        Integer pageInt = Integer.parseInt(page);

        FirewallDirFacade facade = new FirewallDirFacade();
        int totalRows = facade.getTotalFirewallInfo(name, "");
        ...
        if (pageInt < 1) {
            pageInt = 1;
            return ("redirect:/firewallDir/init?page=" + pageInt);
        ...
        List<FirewallDirBO> lst = facade.findAllFirewallInfo(startRow, endRow, name, "");
...
//    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                FirewallDirFacade facade = new FirewallDirFacade();
                List<FirewallDirBO> cpBO = facade.findAllFirewallInfo(1, 10, null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") FirewallDirBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request)  {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/firewallDir/preAdd";
            }
            FirewallDirFacade coreFacade = new FirewallDirFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addFirewallInfo(item, user.getId());
        ...
        return "redirect:/firewallDir/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") FirewallDirBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request)  {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/firewallDir/view/" + item.getId();
            }
            FirewallDirFacade coreFacade = new FirewallDirFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateFirewallInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            FirewallDirFacade coreFacade = new FirewallDirFacade();
            List<FirewallDirBO> firewallDirInfo = coreFacade.findAllFirewallInfo(1, 10, "", nodeId);
            if (firewallDirInfo.size() > 0) {
                UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
                coreFacade.deleteFirewallInfo(Long.valueOf(nodeId), user.getId());
