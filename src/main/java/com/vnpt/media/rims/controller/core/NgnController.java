 ...
 */
@Controller
@RequestMapping(value = "/ngn")
public class NgnController {

    private static Logger logger = LogManager.getLogger(NgnController.class);

    private static final String LIST = "core/ngn/list";
    private static final String VIEW = "core/ngn/edit";
    private static final String POPUP = "core/ngn/popup";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init ngn");
        name = name == null ? "" : name;
        page = page == null ? "1" : page;
        Integer pageInt = Integer.parseInt(page);

        CoreFacade facade = new CoreFacade();
        int totalRows = facade.getTotalNgnInfo(name, "");
        ...
        if (pageInt < 1) {
            pageInt = 1;
            return ("redirect:/ngn/init?page=" + pageInt);
        ...
        List<NgnInfoBO> lst = facade.findAllNgnInfo(startRow,endRow, name, "");
...
@RequestMapping(value = "/popup/{nodeId}", method = RequestMethod.GET)
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CoreFacade facade = new CoreFacade();
                List<NgnInfoBO> cpBO = facade.findAllNgnInfo(1, 10, null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") NgnInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/ngn/preAdd";
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addNgnInfo(item, user.getId());
        ...
        return "redirect:/ngn/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") NgnInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/ngn/view/" + item.getNodeId();
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateNgnInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CoreFacade coreFacade = new CoreFacade();
            List<NgnInfoBO> ngnInfo = coreFacade.findAllNgnInfo(1, 10, "", nodeId);
            if (ngnInfo.size() > 0) {
                coreFacade.deleteNgnInfo(Long.valueOf(nodeId));
