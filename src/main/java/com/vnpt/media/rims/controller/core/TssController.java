...
public class TssController {
    ...
    private static Logger logger = LogManager.getLogger(TssController.class);

    private static final String LIST = "core/tss/list";
    private static final String VIEW = "core/tss/edit";
    private static final String POPUP = "core/tss/popup";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init tss");
        name = name == null ? "" : name;
        page = page == null ? "1" : page;
        Integer pageInt = Integer.parseInt(page);

        CoreFacade facade = new CoreFacade();
        int totalRows = facade.getTotalTssInfo(name, "");
        ...
        if (pageInt < 1) {
            pageInt = 1;
            return ("redirect:/tss/init?page=" + pageInt);
        ...
        List<TssInfoBO> lst = facade.findAllTssInfo(startRow,endRow, name, "");
...
@RequestMapping(value = "/popup/{nodeId}", method = RequestMethod.GET)
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CoreFacade facade = new CoreFacade();
                List<TssInfoBO> cpBO = facade.findAllTssInfo(1, 10, null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") TssInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/tss/preAdd";
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);
            coreFacade.addTssInfo(item, user.getId());
        ...
        return "redirect:/tss/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") TssInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/tss/view/" + item.getNodeId();
            }
            CoreFacade coreFacade = new CoreFacade();
            UserBO user = (UserBO) request.getSession().getAttribute(Constants.USER_KEY);

            coreFacade.updateTssInfo(item, user.getId());
            attr.addFlashAttribute("info", new Message(Message.TYPE_SUCCESS, Message.HEAD_SUCCESS, Message.MESSAGE_UPDATE_SUCCESS));
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CoreFacade coreFacade = new CoreFacade();
            List<TssInfoBO> tssInfo = coreFacade.findAllTssInfo(1, 10, "", nodeId);
            if (tssInfo.size() > 0) {
                coreFacade.deleteTssInfo(Long.valueOf(nodeId));
