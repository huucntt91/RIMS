...
public class CrbtController {
    ...
    private static Logger logger = LogManager.getLogger(CrbtController.class);

    private static final String LIST = "core_ps/crbt/list";
    private static final String VIEW = "core_ps/crbt/edit";
    @Autowired
    ...
    public String init(ModelMap mm,
        ...
        logger.info("Action init crbt");
        name = name == null ? "" : name;

        CorePSFacade facade = new CorePSFacade();

        List<CrbtInfoBO> lst = facade.findAllCrbtInfo(name, "");
        mm.put("list", lst);
        mm.put("name", name);
    ...
    public String preAdd(ModelMap mm, RedirectAttributes attr) throws Exception {
        CrbtInfoBO item = new CrbtInfoBO();
        item = (CrbtInfoBO) attr.getFlashAttributes().get("item");

        mm.addAttribute("item", item == null ? new CrbtInfoBO() : item);
        return VIEW;
    }
    ...
    public String view(@PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            if (StringUtils.hasText(nodeId)) {
                CorePSFacade facade = new CorePSFacade();
                List<CrbtInfoBO> cpBO = facade.findAllCrbtInfo(null, nodeId);
    ...
    public String add(ModelMap mm, @Valid @ModelAttribute(value = "item") CrbtInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            coreFacade.addCrbtInfo(item, user.getId());
        } catch (Exception e) {
        ...
        return "redirect:/crbt/init";

    }

    @RequestMapping(value = "/view/update", method = RequestMethod.POST)
    public String update(ModelMap mm, @Valid @ModelAttribute(value = "item") CrbtInfoBO item, BindingResult bindingResult, RedirectAttributes attr, HttpServletRequest request) throws Exception {
        ...
        try {
            if (bindingResult.hasErrors()) {
                ...
                mm.addAttribute("item", item);
                return "redirect:/crbt/view/" + item.getNodeId();
            }
            ...
            coreFacade.updateCrbtInfo(item, user.getId());
        } catch (Exception e) {
            String message = StringUtils.captureStackTrace(e);
            logger.error("Exception :", e);
            String msg = messageSource.getMessage("admin.common.error", null, locale);
    ...
    public String delete(ModelMap mm, @PathVariable(value = "nodeId") String nodeId,
        ...
        try {
            CorePSFacade coreFacade = new CorePSFacade();
            List<CrbtInfoBO> ggsnInfo = coreFacade.findAllCrbtInfo("", nodeId);
            if (ggsnInfo.size() > 0) {
                coreFacade.deleteCrbtInfo(Long.valueOf(nodeId));
