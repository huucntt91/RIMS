...
public class DsrDAO extends GenericDAO implements IDsr {

    private static Logger logger = LogManager.getLogger(DsrDAO.class);
    ...
    public List<DsrInfoBO> findAllDsrInfo(String name, String id) throws DAOException {
        ...
        try {
            ...
            String querySql = "{? = call PKG_DSR_INFO.fc_find_all(?,?) }";
//       
...
//                    item.setLoaiTramId(rs.getLong("LOAI_TRAM_ID"));
    ...
    @Override
    public int addDsrInfo(DsrInfoBO item, Long userInsert) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
...
// prs_hoan_canh_ra_doi VARCHAR2, prs_ngay_dang_ky DATE, prs_type VARCHAR2
            String querySql = "{? = call PKG_DSR_INFO.fc_insert(?,?,?,?,?,?,?,?,?,?,?,?"
                    + ",?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
//       
    ...
    @Override
    public int updateDsrInfo(DsrInfoBO item, Long userInsert) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_DSR_INFO.fc_update(?,?,?,?,?,?,?,?,?,?,?,?"
                    + ",?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) }";
//       
    ...
    public int deleteDsrInfo(Long id) throws DAOException {
        ...
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_DSR_INFO.fc_delete(?) }";
//       
