...
public class LoaiTuNguonDAO extends GenericDAO implements ILoaiTuNguon {

    private static Logger logger = LogManager.getLogger(LoaiTuNguonDAO.class);

    @Override
    public List<LoaiTuNguonBO> findAll(String id) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_LOAI_TU_NGUON.fc_find_all(?) }";
//       
            ...
            List<?> list = sqlTemplate.queryFunction(querySql, new RowMapper() {
                ...
                public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
                    LoaiTuNguonBO item = new LoaiTuNguonBO();
                    item.setId(rs.getLong("loai_tu_nguon_id"));
                    item.setName(rs.getString("ten_loai_tu_nguon"));
                    return item;
                }   
            }, vars);

            return (List<LoaiTuNguonBO>) list;
    ...
    @Override
    public int modify(String action, LoaiTuNguonBO loaiTuNguonBO) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_LOAI_TU_NGUON.fn_modify(?,?,?)}";
            if (loaiTuNguonBO == null) {
                return -1;
            }
            ...
            List<Object> vars = new Vector<Object>();
            vars.add(action);
            vars.add(loaiTuNguonBO.getId());
            vars.add(loaiTuNguonBO.getName());
