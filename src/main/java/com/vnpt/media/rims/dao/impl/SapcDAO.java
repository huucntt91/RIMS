...
public class SapcDAO extends GenericDAO implements ISapc {

    private static Logger logger = LogManager.getLogger(SapcDAO.class);
    ...
    public List<SapcInfoBO> findAllSapcInfo(String name, String id) throws DAOException {
        ...
        try {
            ...
            String querySql = "{? = call PKG_SAPC_INFO.fc_find_all(?,?) }";
//       
...
//                    item.setCardVersion(rs.getString("card_version"));
    ...
    @Override
    public int addSapcInfo(SapcInfoBO item, Long userInsert) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
...
// prs_hoan_canh_ra_doi VARCHAR2, prs_ngay_dang_ky DATE, prs_type VARCHAR2
            String querySql = "{? = call PKG_SAPC_INFO.fc_insert(?,?,?,?,?,?,?,?,?,?,?,?"
                    + ",?,?,?,?,?,?,?,?,?,?) }";
//       
...
//            vars.add(item.getCardSL());
    ...
    @Override
    public int updateSapcInfo(SapcInfoBO item, Long userInsert) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_SAPC_INFO.fc_update(?,?,?,?,?,?,?,?,?,?,?,?"
                    + ",?,?,?,?,?,?,?,?,?,?) }";
//       
...
//            vars.add(item.getCardSL());
    ...
    public int deleteSapcInfo(Long id) throws DAOException {
        ...
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_SAPC_INFO.fc_delete(?) }";
//       
