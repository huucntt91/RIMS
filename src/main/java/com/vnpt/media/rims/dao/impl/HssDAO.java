...
public class HssDAO extends GenericDAO implements IHss {

    private static Logger logger = LogManager.getLogger(HssDAO.class);
    ...
    public List<HssInfoBO> findAllHssInfo(int startRow, int endRow, String name, String id) throws DAOException {
        ...
        try {
            ...
            String querySql = "{? = call PKG_HSS_INFO.fc_find_all(?,?,?,?) }";
//       
...
//                    item.setLoaiTramId(rs.getLong("LOAI_TRAM_ID"));
    ...
    @Override
    public int getTotalHssInfo(String name, String id) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_HSS_INFO.fc_find_total(?,?) }";
//       
    ...
    @Override
    public int addHssInfo(HssInfoBO item, Long userInsert) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
...
// prs_hoan_canh_ra_doi VARCHAR2, prs_ngay_dang_ky DATE, prs_type VARCHAR2
            String querySql = "{? = call PKG_HSS_INFO.fc_insert(?,?,?,?,?,?,?,?,?,?,?,?"
                    + ",?,?,?) }";
//       
    ...
    @Override
    public int updateHssInfo(HssInfoBO item,Long userInsert) throws DAOException {
        Connection conn = null;
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_HSS_INFO.fc_update(?,?,?,?,?,?,?,?,?,?,?,?"
                    + ",?,?,?) }";
//       
    ...
    public int deleteHssInfo(Long id) throws DAOException {
        ...
        try {
            conn = this.getConnection();
            String querySql = "{? = call PKG_HSS_INFO.fc_delete(?) }";
//       
