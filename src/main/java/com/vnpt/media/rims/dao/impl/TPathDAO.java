...
public class TPathDAO extends GenericDAO implements ITPath {
    
    private static final String RIMS_BB_DS = ResourceBundle.getBundle("config", Locale.getDefault()).getString("RIMS_BB_DS");

    private static Logger logger = LogManager.getLogger(TPathDAO.class);
    ...
    public List<TPathInfoBO> findAllTPathInfo(int startRow, int endRow, String name, String id) throws DAOException {
        ...
        ArrayList<TPathInfoBO> arrayList = new ArrayList<>();
        try {
            conn = EnvManager.getDbConnection(RIMS_BB_DS);
            String sql = "begin ?:= pkg_tpath.fn_find_tPath(?,?,?,?); end;";
    ...
    @Override
    public int getTotalTPathInfo(String name, String id) throws DAOException {
        CallableStatement cstmt = null;
        Connection conn = null;
        try {
            conn = EnvManager.getDbConnection(RIMS_BB_DS);
            String sql = "begin ?:= pkg_tpath.fc_find_total(?,?); end;";
            cstmt = conn.prepareCall(sql);
    ...
    @Override
    public int addTPathInfo(TPathInfoBO item, Long userInsert) throws DAOException {
        CallableStatement cstmt = null;
        Connection conn = null;
        try {
            conn = EnvManager.getDbConnection(RIMS_BB_DS);
            String sql = "begin ?:=pkg_tpath.fn_insert(?,?,?,?,?); end;";
            cstmt = conn.prepareCall(sql);
            cstmt.registerOutParameter(1, oracle.jdbc.OracleTypes.INTEGER);
            cstmt.setString(2, item.gettPathId());
            cstmt.setString(3, item.gettConnectionId());
            cstmt.setString(4, item.gettPathName());
    ...
    @Override
    public int updateTPathInfo(TPathInfoBO item,Long userInsert) throws DAOException {
        CallableStatement cstmt = null;
        Connection conn = null;
        try {
            conn = EnvManager.getDbConnection(RIMS_BB_DS);
            String sql = "begin ?:=pkg_tpath.fn_update(?,?,?,?,?); end;";
            cstmt = conn.prepareCall(sql);
            cstmt.registerOutParameter(1, oracle.jdbc.OracleTypes.INTEGER);
            cstmt.setString(2, item.gettPathId());
            cstmt.setString(3, item.gettConnectionId());
    ...
    public int deleteTPathInfo(Long id, Long userInsert) throws DAOException {
        ...
        try {
            conn = EnvManager.getDbConnection(RIMS_BB_DS);
            String sql = "begin ?:=pkg_tpath.fn_delete(?,?); end;";
            cstmt = conn.prepareCall(sql);
            cstmt.registerOutParameter(1, oracle.jdbc.OracleTypes.INTEGER);
